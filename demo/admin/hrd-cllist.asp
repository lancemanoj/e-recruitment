...
<style>
    /* Ensure table columns are aligned */
    .table-wrapper table {
        table-layout: fixed;
        width: 100%;
    }

    /* Adjust checkbox cell width */
    .table-wrapper th, .table-wrapper td {
        padding: 10px; /* Add some padding to make alignment better */
        text-align: center;
    }
    ...
    .table-wrapper td input[type="checkbox"] {
        ...
        vertical-align: middle; /* Align the checkbox vertically with text */
    }

    /* Optional: If the checkbox column is too wide, you can set a specific width for it */
...
<script type="text/javascript">
    $(document).ready(function () {
        // Initialize stickyTableHeaders plugin on specific table
        $(".table-wrapper").stickyTableHeaders();

        // Function to add borders to the table header and body within the .table-wrapper
        function addBordersToTable() {
            var $table = $(".table-wrapper table");

            // Add borders to header rows and cells
            $table.find("thead tr").css('border', '2px solid black');
            $table.find("thead th").css('border', '1px solid black');

            // Add borders to body rows and cells
            $table.find("tbody tr").css('border', '1px solid blue');
            $table.find("tbody td").css('border', '1px solid blue');

            // Ensure that every column gets the border (even the first few)
        ...
        // Call the function to add borders after sticky headers are applied
        $(".table-wrapper").on('stickyTableHeaders:stuck', function () {
            addBordersToTable();
        });

        // Optionally, call it immediately for any existing sticky tables
        addBordersToTable();

        // Tooltip functionality for elements with masterTooltip class
