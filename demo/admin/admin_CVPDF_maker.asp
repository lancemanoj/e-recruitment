...
'Response.CharSet = "UTF-8"
	...
	Dim VACNUM, VCDate, VacInfosql, VacInfo, objASPError, counter, pv_edulinecount, pv_new_sessioncode
	dim pv_prefFname, pv_prefLnam, pv_realFname, pv_realLname, pv_honor
...
'29 SEP 24 LJL added DIVERSITY SECTION HEADER
'29 SEP 24 LJL modified to not remove simple space, only more than one
'03 OCT 24 LJL revised cand_gnd_i for GetDEI to be JAPINFO1 instead
'05 OCT 24 LJL moved law question to later section for all UN orgs
'06 OCT 24 LJL many updates to new legal questions and fixed wrong terminated field
'05 OCT 24 LJL removed the loop for the include text for DIV.  Not sure why it was there.			
'05 OCT 24 LJL removed extra code as this is just a few lines above
'06 OCT 24 LJL added page break css to force a break between applicant sections		
'08 OCT 24 LJL compare diversity which doesn't show in TOC to Verif that does show but has a line through it??
'08 OCT 24 LJL move cand name to left side
'09 OCT 24 LJL NAME ALIGNMENT AND HEADER TITLE
'09 OCT 24 LJL rev Div to DI and added output single or double.
'10 OCT 24 LJL fix this include text
'10 OCT 24 LJL maybe change back
'10 OCT 24 LJL changed contact details table back to no <br>
'14 OCT 24 LJL remove extra law question output as it is moved to Verification section.--->
'14 OCT 24 LJL not family/relatives for IFRC
'14 OCT 24 LJL clean up page
'15 OCT 24 LJL don't show diversity populations if nothing in it
' 17 OCT 24 LJL only show UN typing results if yes to answer
' 17 OCT 24 LJL marking where typing results are
' 17 OCT 24 LJL not show language typing lines if no entries
' 17 OCT 24 LJL added line between employ lines
' 18 OCT 24 LJL employment line title color				
' 18 OCT 24 LJL remove faxes					
' 20 OCT 24 moved int exp regions if yes
' 20 OCT 24 LJL revise stay in town and ctry
' 20 OCT 24 LJL moved diversity applicant info from top to inside loop, but add to stored proc's
' 20 OCT 24 LJL added cand Url	
' 23 OCT 24 LJL disability is reversed 0 = yes and 1 = no per Manoj
' 23 OCT 24 LJL show disability accommodations text if said yes
' 23 OCT 24 LJL remove nat2 and 3 if no nat2
' 25 OCT 24 LJL check and show preferred name(s) and then real names if present
' 28 OCT 24 LJL if any accommodation text, print out



	'14 OCT 24 LJL remove or add table borders as formatting
	dim pv_tableborder
...
' SET TO DEBUG TO  1 or not REM
'PV_DEBUG = 1


Dim gITEXTPHsql, gITEXTPH, tempval1, obj_int_select_CmdII, obj_int_select_CmdDV
Set obj_int_select_CmdII = Server.CreateObject("ADODB.Command")
obj_int_select_CmdII.ActiveConnection  = rsys_int_select
Set obj_int_select_CmdDV = Server.CreateObject("ADODB.Command")
obj_int_select_CmdDV.ActiveConnection  = rsys_int_select

if pv_DEBUG = 1 then
	response.write "<br>test maker"
	response.write "<br>stop 1.1"
	'response.End()
end if 	
...
'<<--Modified by Interface on 07/25/2007
' 25 OCT 24 LJL addd official name text for preferred vs official DIV
gITEXTAsql = "SELECT i_1, i_2, i_3, i_4, i_5, i_6, i_7, i_8, i_9, i_10, i_11, i_14, i_15, i_16, i_19, i_20, i_21, i_22, i_23, i_26, i_27, i_28, i_29, i_30, i_38, i_40, i_48, i_49, i_67, i_68, i_69, i_70, i_72, i_73, i_74, i_76, i_77, i_79, i_81, i_89, i_96, i_97, i_99, i_text7, i_text8 FROM tr_rsys_itext WHERE itext_thisorg_c = ? AND itext_lng_c = ? AND itext_page_c = 'A' "
...
if instr(pv_parts,"J,") then
	...
	gITEXTJsql = " SELECT i_1, i_3,i_8, i_12, i_13, i_71 FROM tr_rsys_itext WHERE itext_thisorg_c = ? AND itext_lng_c = ? AND itext_page_c = 'J' "
...
if instr(pv_parts,"GR,") then
	...
	gITEXTGRefsql = "SELECT i_1, i_15,i_30,i_5,  i_17, i_18, i_19, i_57, i_27,i_80,i_75,i_19  FROM tr_rsys_itext WHERE itext_thisorg_c = ? AND itext_lng_c = ? AND itext_page_c = 'G' "
...
if instr(pv_parts,"T,") then
	Dim gITEXTTsql, gITEXTT, iTotalRecordsT
	'<<--Modified by Interface on 07/25/2007
...
'09 OCT 24 LJL added to test Diversity query output
'	iTotalRecordsT = gITEXTT.RecordCount

'	f.WriteLine "<br>T SQL: " & gITEXTTsql & " -- END "
'	f.WriteLine "<br>T RC: " & iTotalRecordsT & " -- END "	
'if gITEXTT.eof = false then
'		f.WriteLine "<br>T EOF: " & gITEXTT.eof & " -- END "	
'	else
'		f.WriteLine "<br>T EOF: <b>NOT WORKING</b> "	
'	end if
'	f.WriteLine "<br>T CHECK: YEP -- END "	

	'-->>
end if

' BEGIN CHECK IF PERSON SELECTED SECTION DIV6
if instr(pv_parts,"DV,") then
	Dim gITEXTDeisql, gITEXTDei
	'dim iTotalRecordsDV
	'<<--Modified by Interface on 07/25/2007

	gITEXTDeisql = "SELECT i_1, i_9, i_31, i_32, i_83, i_84, i_87, i_91, i_97, i_text3 FROM tr_rsys_itext WHERE itext_thisorg_c = ? AND itext_lng_c = ? AND itext_page_c = 'Div' "
	obj_int_select_CmdDV.CommandText = gITEXTDeisql
	Set gITEXTDei = obj_int_select_CmdDV.Execute(,Array(pv_new_sessioncode,session("lng")))

'09 OCT 24 LJL added to test Diversity query output
'	iTotalRecordsDV = gITEXTDei.RecordCount
	
'	f.WriteLine "<br><br><b>DEI SQL: " & gITEXTDeisql & " -- END </b>"
'	f.WriteLine "<br>DEI RC: " & iTotalRecordsDV & " -- END "	
	
'if gITEXTDei.eof = false then
'		f.WriteLine "<br>DEI EOF: " & gITEXTDei.eof & " -- END "	
'	else
'		f.WriteLine "<br>DEI EOF: <b>NOT WORKING</b> "	
'	end if
		
'	f.WriteLine "<br>DEI CHECK: YEP -- END "	
'	response.write "<br><br><b>DEI SQL: " & gITEXTDeisql & " -- END </b>"
'	response.write "<br>DEI RC: " & iTotalRecordsDV & " -- END "	
'if gITEXTDei.eof = false then
'		response.write "<br>DEI EOF: " & gITEXTDei.eof & " -- END "	
'	else
'		rresponse.write "<br>DEI EOF: <b>NOT WORKING</b> "	
'end if
'	response.write "<br>DEI EOF: " & gITEXTDei("i_1") & " -- END "	
'	response.write "<br>DEI CHECK: YEP -- END "	
...
'response.write "<br>FUCKKIT"
'response.end

if pv_DEBUG = 1 then
	response.write "<br>test maker"
	response.write "<br>stop 1.2"
	'response.End()
end if 	
...
'f.WriteLine  " H2   {"
'f.WriteLine  "  FONT-SIZE: 12pt; margin-bottom: 40px;"
'f.WriteLine  " }"
...
'04 mar 07 - ac - get job title and VN number
'08 OCT 24 LJL add replace to put VNumber on next line from VN title in PDF front page
VNTitle = " "
VNNumber = " "
VN = " "
VCDate = " "
if len(pv_jobid) then
	...
	if  VacInfo.eof = false then
		...
		VNTitle = VacInfo("jobinfo_job_en_t") 
		VNNumber = VacInfo("jobinfo_vac2_c")
...
if pv_DEBUG = 1 then
	response.write "<br>test maker"
	response.write "<br>stop 3.3"
	'response.End()
end if 	


'09 JUL 15 GG Added French for Dates
if new_lng_code = "fr" then Session.LCID = 4108 
	'French - Switzerland
...
' <!---------------- BLOCK 1 START 
if instr(request.servervariables("HTTP_REFERER"),"/admin/admin_CVPDF_docsettings.asp") then

	if pv_DEBUG = 1 then
		response.write "<br>test maker"
		response.write "<br>stop 3.5"
		'response.End()
	end if 	
	...
	if pv_DEBUG then
		...
		response.write "<br>test maker"
		response.write "<br>stop 3.8"
		' response.End()
	end if

	'19 feb 07 ac - added new temp /public_hold when fixed changed back /public
' <!---------------- BLOCK 1 MID
elseif instr(request.servervariables("HTTP_REFERER"),"/public/pdf/pub_CVPDF_docsettings.asp") then

	if pv_DEBUG = 1 then
		response.write "<br>test maker"
		response.write "<br>stop 4.1"
		'response.End()
	end if 	
...
if pv_multicode <> "" OR applicant_id <> "" then
	...
	if pv_DEBUG = 1 then
		response.write "<br>test maker"
		response.write "<br>stop 5.1"
		response.End()
	end if 	

	response.write "<Br><font color='maroon'>IMPROPER ENTRY - NO CAND ID, please report how you entered this page to tech support <a href='mailto:erecruit.support@talentisoft.com'>Tech Support</a></font>"
	response.end
end if
...
if new_lng_code = "" OR pv_new_sessioncode = "" then
	response.write "<br><font face=arial>Cannot proceed.  Missing instruction (error: 111)" & "new_lng_code=" & new_lng_code & "Org. Code=" & pv_new_sessioncode & "</font><br>"
	response.end
end if
...
if pv_multi = "1" then
		' INFO multicode comes across as VARCHAR from the querystring, to get the results of the storage table of cand_id's
...
liner = liner + 1
	...
	' 08 OCT 24 LJL set footer info to be VNTitle only
	f.WriteLine "<!-- FOOTER CENTER """ & VNTitle & """ -->"
	'09 OCT 24 LJL remove extra space before sections from big name bloc
	'f.WriteLine "<br>"

	' 08 OCT 24 LJL move cand name to left side
	f.WriteLine "<h3 style=""text-align: left;"">" 
	Randomize
	intNumber = Int((intHighNumber - intLowNumber + 1) * Rnd + intLowNumber)

'16 DEC 20 LJL added goword check to stop the CAN (PDF chapter marker) from showing in word output 
' 25 OCT 24 LJL add preferred names

	' WORD VERSION ----------------------------------------------------->
	
if request.form("goWord") = "99" then
	f.WriteLine "<a style=""abcpdf-tag-visible: true;text-align: left;display:none;"" id='" & trim(Ucase(JAPINFO1("cand_lnam_t"))) & ", " & JAPINFO1("cand_fnam_t")  & "_|"  & intNumber  & "_h1'></a>" 
else

	' PDF VERSION ----------------------------------------------------->
	
	' 25 OCT 24 LJL check for and show preferred name if present
	
	pv_prefFname = trim(Ucase(JAPINFO1("cand_ofnam_t"))) 
	pv_realFname = trim(Ucase(JAPINFO1("cand_fnam_t")))
	
	pv_prefLname = trim(Ucase(JAPINFO1("cand_onam_t"))) 
	pv_realLname = trim(Ucase(JAPINFO1("cand_lnam_t")))
	
	if len(pv_prefFname) then
		pv_Fname = pv_prefFname
	else
		pv_Fname = pv_realFname
	end if
	
	if len(pv_prefLname) then
		pv_Lname = pv_prefLname
	else
		pv_Lname = pv_realLname
	end if

	
	' WRITE TOP NAME 
	
	f.WriteLine "<a style=""abcpdf-tag-visible: true;text-align: left;display:none;"" id='" & pv_Lname & ", " & pv_Fname & "_|"  & intNumber  & "_h1'>CAN</a>" 

end if
	 
	 ' response.write "<br><br>END POINT 777"
	 ' response.end
	 ...
	 ' 25 OCT 24 LJL preferred names addition
		 f.WriteLine pv_Lname & ", " & pv_Fname
...
if instr(pv_parts,"A,") then
	...
	' f.WriteLine "<br/>"
	f.WriteLine "<p><br></p>"
	f.WriteLine "<h2 style=""margin-bottom: 100px;""><font color=""" & g_headerColor & """>" & gITEXTA("i_1") & "</font></h2>"
	...
	if pv_pixok = "1" then
		...
		if pf.FileExists(src_pix_loc) then
			pf.CopyFile src_pix_loc, dest_dir
						 '09 OCT 24 LJL revise the photo table and out put , remove extra border, center
						 f.WriteLine "<tr>"
						 f.WriteLine "<td colspan=""2"" valign=""top"" align=""center"">"
						 f.WriteLine "<table width=""100%"" border=""0"" align=""center""><tr align=""center""><td align=""center"" valign=""top"">"
						 f.WriteLine "<img src=""" & GETPIC("upl_snam_t") & """ border=0 alt="""" width=""150"">"
                          f.WriteLine "your logo here"
	...
	' 25 OCT 24 LJL no more WHO and UNAIDS auto internal

' ----------------------------------------------------------------------------------------------																
' SECTION A WHO or UNAIDS  DISABLED
' ----------------------------------------------------------------------------------------------																
if 0=1 then 
	' (pv_new_sessioncode = 1000) OR pv_new_sessioncode = 1500 by Gevorg
	' 17 APR 07 LJL modif to properly assess if truly internal, mainly WHO  - thisorg_stafftrue is the staff number from WHO. If somethng there, then they are in WHO internal dir
	...
	if len(JAPINFO1("thisorg_stafftrue")) then
		...
		if len(JAPINFO1("cand_mnam_t")) OR len(JAPINFO1("cand_onam_t")) then
			...
			if pv_new_sessioncode <> 7000 and len(JAPINFO1("cand_onam_t")) then
				f.WriteLine "<td valign=""top"">" & gITEXTA("i_96") & " <b>" & JAPINFO1("cand_onam_t")& "</b></td>"
			else
				f.WriteLine "<td>&nbsp;</td>"
			end if
		...
		f.WriteLine "</tr>"
		' Start change by Atul make colspan=2
		f.WriteLine "<tr><td valign=""top"" colspan=""2"">" & gITEXTA("i_16") & " <b>" & JAPINFO1("s_nat") & "</b></TD></tr>"
		' IS OR IS NOT INTERNAL AND NOT WHO 1000
		
		'25 OCT 24 LJL END INTERNAL WHO AND unaids ----------------------------------------------------
		
	' ----------------------------------------------------------------------------------------------																
	' SECTION A WHO or UNAIDS NOT DIRECT CONNECT
	' ----------------------------------------------------------------------------------------------																
	else
		...
		' 25 OCT 24 LJL add preferred and real names
		f.WriteLine "<tr><td valign=""top"" align=""center"" colspan=""2""><strong>" & JAPINFO1("honordsc") & " " & pv_realLname & ")) " & pv_realFname & "</strong></td></tr>"
		...
		if  len(JAPINFO1("cand_mnam_t")) OR len(JAPINFO1("cand_onam_t")) then
			...
			' 25 OCT 24 LJL add preferred and real names
			if len(pv_prefLname) then
				f.WriteLine "<td valign=""top"">" & gITEXTA("i_96") & " +++<b>" & pv_prefLname & "</b></td>"
			else
				f.WriteLine "<td>&nbsp;</td>"
			end if
	
			if len(pv_prefFname) then
				f.WriteLine "<td valign=""top"">" & gITEXTA("i_98") & " ///<b>" & pv_prefFname & "</b></td>"
			else
				f.WriteLine "<td>&nbsp;</td>"
			end if
		...
		' 25 OCT 24 LJL add other gender			
		If JAPINFO1("cand_gnd_i") = 1  then
			f.WriteLine "<td valign=""top"">" & gITEXTA("i_5") & " <b>" & gITEXTA("i_29") & "</b></td>"
		elseif JAPINFO1("cand_gnd_i") = 0 then
			f.WriteLine  "<td valign=""top"">" & gITEXTA("i_5") & " <b>" & gITEXTA("i_30") & "</b></td>"
		elseif JAPINFO1("cand_gnd_i") = 3 then
			f.WriteLine  "<td valign=""top"">" & gITEXTA("i_97") & " <b>" & gITEXTA("i_30") & "</b></td>"
		end if
...
' -------------------------------------------------------------------------------------------------------------												
' SECTION A EXTERNAL CAND SECTION (not WHO or UNAIDS) ALL OTHERS
' -------------------------------------------------------------------------------------------------------------											
else
		' Commented by Gevorg
		...
		if len(trim(JAPINFO1("honordsc"))) then
			pv_honor = trim(JAPINFO1("honordsc")) & " "
		else 
			pv_honor = ""
		end if
		
		' show passport name
		' if len(pv_realFname) then
				f.WriteLine "<td valign=""top"">" & gITEXTA("i_text8") & " <b>" & pv_honor & pv_realFname & " " & pv_realLname & "</b></td>"
		' end if

		' show preferred name
		' if len(pv_prefFname) then
				f.WriteLine "<td valign=""top"">" & gITEXTA("i_text7") & " <b>" & pv_honor & pv_prefFname & " " & pv_prefLname & "</b></td>"
		' else
		...
		' f.WriteLine "<tr><td valign=""top"" align=""center"" colspan=""2""><strong>///" 
				
		' 25 OCT 24 LJL check for preferred first name
				
		' if len(pv_prefFname) then
		'	f.WriteLine pv_prefFname & " "
		' else
		'	f.WriteLine pv_realFname & " "
		' end if

		' 25 OCT 24 LJL check for preferred last name
		'if len(pv_prefLname) then
		'	f.WriteLine pv_prefLname & " "
		' else
		'	f.WriteLine pv_realLname & " "
		' end if
				
		' 25 OCT 24 LJL error occurs here not
		' f.WriteLine "<tr><td valign=""top"" colspan=""2"">"
		...
		elseIf JAPINFO1("cand_gnd_i") = 0  then
			f.WriteLine  "<td valign=""top"">" & gITEXTA("i_5") & " <b>" & gITEXTA("i_30") & "</b></td>"
		elseIf JAPINFO1("cand_gnd_i") = 3  then
			f.WriteLine  "<td valign=""top"">" & gITEXTA("i_5") & " <b>" & gITEXTA("i_97") & "</b></td>"
		end if
		...
		' Start chnage by Atul make colspan=2-->
		' 25 OCT 24 LJL change all nats to one line
		f.WriteLine "<tr><td valign=""top"" colspan=""2"">" & gITEXTA("i_49")& " <b>" &  JAPINFO1("natcty")
		...
		if pv_new_sessioncode = 1000 or pv_new_sessioncode = 1500 then
			...
			if JAPINFO1("thisorg_stafftrue") > 0 then
				...
				if isdate(JAPINFO1("contract_end_date")) then
					f.WriteLine day(JAPINFO1("contract_end_date")) & "-" & monthname(month(JAPINFO1("contract_end_date")),1) & "-" & yea (JAPINFO1("contract_end_date"))
				end if
				f.WriteLine "</strong></td>"
				f.WriteLine "</tr>"
			else
				if JAPINFO1("thisorg_short") = 1 then
					...
					if isdate(JAPINFO1("thisorg_end")) then
						f.WriteLine "<br><strong>" & day(JAPINFO1("thisorg_end")) & "-" & monthname(month(JAPINFO1("thisorg_end")),1) & "-" & yea (JAPINFO1("thisorg_end"))
					end if
					f.WriteLine "</strong></td>  "
					f.WriteLine "</tr>"
				end if
			...
			if JAPINFO1("thisorg_short") = 1 then
				...
				if pv_new_sessioncode = 2900 then
					...
					if isdate(JAPINFO1("thisorg_start")) then
						f.WriteLine "<br><strong>" & day(JAPINFO1("thisorg_start")) & "-" & monthname(month(JAPINFO1("thisorg_start")),1) & "-" & yea (JAPINFO1("thisorg_start"))
					end if
...
if instr(pv_parts,"W,") then
	...
	' 09 OCT 24 LJL added <br> to all header <table under </h2>	
	' 10 OCT 24 LJL changed contact details table back to no <br>
	' f.WriteLine "<TABLE border=""0"" bordercolor=""black"" cellpadding=""1"" width=""" & widther & """ align=""center"">"
...
' End chnage by atul remove the hr  make line with tr and table -->
			...
			f.WriteLine "<TABLE border=""0"" bordercolor=""red"" width=""100%"">"
			
			f.WriteLine "<td width=""50%"" valign=""top"">"
			' until fax number
			f.WriteLine "<table border=""0"" bordercolor=""orange"">"
			f.WriteLine "<tr><td valign=""top"">" & gITEXTI("i_3") & "</TD></tr>"
			f.WriteLine "<tr><td valign=""top""><b>" & japinfo1("cand_padd_m") & "<br>" & JAPINFO1("cand_pcity_t") & "<br>" & JAPINFO1("cand_pzip_t") & "<br>" & JAPINFO1("permcty") & "</b></td></tr>"
			' if len(JAPINFO1("cand_pphn_t")) or len(JAPINFO1("cand_wphn_t")) then
				if len(JAPINFO1("cand_pphn_t")) then
					f.WriteLine  "<TR><td valign=""top""><I>" & gITEXTI("i_16") & "</i> <b>" & JAPINFO1("cand_pphn_t") & "</b></td></tr>"
				else
					f.WriteLine  "<TR><td valign=""top"">" & gITEXTI("i_46") & "</td></tr>"
				end if
				if len(JAPINFO1("cand_wphn_t")) then
					f.WriteLine "<tr><td valign=""top""><I>" & gITEXTI("i_17") & "</I> ++<b>" & JAPINFO1("cand_wphn_t") & "</b></td></tr>"
				else
					f.WriteLine "<tr><td valign=""top""><I>" & gITEXTI("i_46") & "//</I></td></tr>"
				end if
			' end if
		...
		'			end if
			...
			if   len(JAPINFO1("cand_radd_m")) then
				...
				f.WriteLine "<TABLE border=""0"" bordercolor=""red"">"
				...
				if len(JAPINFO1("cand_pphn_2_t")) or len(JAPINFO1("cand_wphn_2_t")) or len(JAPINFO1("cand_mphn_2_t")) then
					...
					if len(JAPINFO1("cand_pphn_2_t")) then
						f.WriteLine "<td valign=""top"">" &  gITEXTI("i_16") & "</i> <b>" & JAPINFO1("cand_pphn_2_t") & "</b></td>"
					else
			...
			'25 OCT 24 LJL mobile moved here
			if len(JAPINFO1("cand_mphn_t")) then
				f.WriteLine "<tr><td valign=""top"" colspan=""2""><I>" & gITEXTI("i_18") & "</I> <b>" & JAPINFO1("cand_mphn_t")& "</b></td></tr>"
			end if
...
'loop
			...
			'15 OCT 24 remove int emp if no 
		If JAPINFOIE("cand_io_f_i") = 0 then  
					' and pv_new_sessioncode = 2900 then
				...
				' IO 1
			if trim(JAPINFOIE("cand_io_f1_t")) <> "" then
				f.WriteLine "<TR>"
			...
			if trim(JAPINFOIE("cand_io_f2_t")) <> "" then
				' IO 2   StripSpecialChar(
				f.WriteLine "<TR>"
				f.WriteLine "<td valign=""top""><strong>" & JAPINFOIE("cand_io_f2_t") & "</strong></TD>"
				f.WriteLine "<td valign=""top""><strong>" & JAPINFOIE("cand_io_f2_ds_t") & "</strong></TD>"
			...
			if trim(JAPINFOIE("cand_io_f3_t")) <> "" then
				' IO 3
				f.WriteLine "<TR>"
...
'</tr>
	...
	' if answer yes to int exp
	If trim(JAPINFOIE("cand_geo_exp_i")) = "1" then
				f.WriteLine gITEXTF("i_6") & "</strong></td></tr>"
				
		'11 JUN 15 GG Added countries for Geographical Experience
		' 20 OCT 24 moved int exp regions if yes
		If JAPGEOLIST.eof = true then
			'f.WriteLine "<TR><td valign=""top"" colspan=""4"">" & gITEXTF("i_21") & "</td></TR>"
		Else
			f.WriteLine "<TR><td valign=""top"" colspan=""4"">"
		...
		' ANSWER NO to INT EXP
				f.WriteLine gITEXTF("i_7") &"</strong></td></tr>"
	...
	End If
			...
			f.WriteLine "<TR><td valign=""top"" colspan=""4"">" & gITEXTF("i_17") & " <strong>" & trim(JAPINFOIE("georesdsc")) & "</strong></TD></TR>"
			
			if trim(JAPINFOIE("georesdsc")) = "Yes" OR trim(JAPINFOIE("georesdsc")) = "Oui" then
...
''------ insertion ------->		
	' 17 JUN 16	GG Added colored line 
				...
				if pv_edulinecount > 0 then	
					f.WriteLine "<span><TABLE border=" & pv_tableborder & " cellpadding=""0"" cellspacing=""0"" bordercolor=""red""><tr><td>&nbsp;</td></tr></table></span>"
				end if

' 18 OCT 24 LJL employment line title color				
				f.WriteLine "<table border=""0"" cellpadding=""1""><tr bgcolor=""#ECFAFF"">"
...
' 18 OCT 24 LJL employment line title color				
				f.WriteLine "<td  width=""100%"" valign=""center""><strong><em>" & UCASE(JAPEMP("candemploy_title_t")) & "</em></strong></td>"
...
'17 OCT 24 LJL added line between employ lines
			' f.WriteLine "<TR>"
			' f.WriteLine "<td valign=""top"" colspan=""4"">____</td>"
			' f.WriteLine "</TR>"
...
' ******************************************************************************
' SECTION G - DEPENDANTS - FAMILY / RELATIVES / REFERENCES 
' ******************************************************************************
' BEGIN CHECK IF PERSON SELECTED SECTION G
		if instr(pv_parts,"G,") then
...
'14 oct 24 LJL removed divider lines
				' f.WriteLine "<tr border=""0"" bgcolor='" & orgColor & "' >"
				' f.WriteLine "<td valign=""top"" colspan=""4"">&nbsp;</td>"
				' f.WriteLine "</tr>"
...
'start chnage by atul remove the hr  make line with tr and table
				...
				' f.WriteLine "<td valign=""top"" colspan=""4"">&nbsp;</td>"
				' f.WriteLine "</tr>"
		' End change by atul remove the hr  make line with tr and table
		JAPREL.movenext
		loop
...
'			    end if
			...
			loop
'01 JUN 15 LJL made table 100% to fill page across for In city since, in country since
			f.WriteLine "</table><table <TABLE border=" & pv_tableborder & " width=100% bordercolor=green>"
...
' ******************************************************************************
' SECTION J - ADDITIONAL INFOR /OTHER INFORMATION
' ******************************************************************************
' BEGIN CHECK IF PERSON SELECTED SECTION J
		f.WriteLine "<!--Test33-->"
if instr(pv_parts,"J,") then
		...
		'10 OCT 24 LJL maybe change back
		'10 OCT 24 LJL check if anything to show as far as Additional/other information

		' 15 OCT 24 LJL START check if any information in this section at all

		' add_info ok 
		' info_m  and linked OK
		' linkedin NOT OK

		' OK
		'	if 0=1 then
		' OK
		' if len(trim(JAPINFO1("cand_add_info_m")))  then
		' DOES APPEAR
		' if len(trim(JJAPINFO1("cand_info_m"))) then
		' DOES APPEAR		
		' if trim(JAPINFO1("cand_linkedin_url")) <> "" then

		' 20 OCT 24 LJL check for addional info
		' if trim(JAPINFO1("cand_add_info_m")) <> "" then  ---OK

		' if trim(JAPINFO1("cand_add_info_m")) <> "" or trim(JAPINFO1("cand_info_m")) <> "" then ---OK

		'if trim(JAPINFO1("cand_linkedin_url")) <> "" then
	
	' 20 OCT 24 LJL added linkedin field to stored procs to make this work right
	if trim(JAPINFO1("cand_add_info_m")) <> "" OR trim(JAPINFO1("cand_info_m")) <> "" OR trim(JAPINFO1("cand_linkedin_url")) <> "" OR trim(JAPINFO1("cand_url_c")) <> "" then
		
		'	if len(trim(JAPINFO1("cand_add_info_m"))) or len(trim(JAPINFO1("cand_info_m"))) OR len(trim(JAPINFO1("cand_linkedin_url"))) then
		'		f.WriteLine "<p><br>len:" & len(trim(JAPINFO1("cand_linkedin_url"))) & "</p>"
		...
		if trim(JAPINFO1("cand_add_info_m")) <> "" then
...
'  '------------ TOOK OUT PDF TESTER 4 ---->
			...
			' 15 BEGIN OCT 24 OK
		if trim(JAPINFO1("cand_info_m")) <> "" then
				'  	' OLD NEEDer#Delta1# -->
		...
		if pv_new_sessioncode = 5500 then
				...
				' 20 OCT 24 LJL added cand Url	
				if trim(JAPINFO1("cand_url_c")) <> "" then
				f.WriteLine "<TR valign=""top""><td valign=""top""><font color=""Teal""><i><strong>" & gITEXTJ("i_8")  & "</strong></I></FONT><br><strong>" & JAPINFO1("cand_url_c") & "</strong></TD></tr>"
				end if
				'13 APR 16 GG added Linkedin Url	
				if trim(JAPINFO1("cand_linkedin_url")) <> "" then
		...
		' 15 BEGIN OCT 24 OK
		if Request.querystring("viewupd") = True and viewupd = "YES"  then
			if GETUPDS("editJ_d") > GETAPPDATE("candjob_d")  then
				f.WriteLine "<TR><td valign=""top"" bgcolor=""" & gITEXTPH("i_62") & """><font color=""" & gITEXTPH("i_63") & """>Last update date <strong>" & formatdatetime(GETUPDS("EditJ_d"),1) & " AFTER APPLYING</strong></TD></tr>"
			end if
		end if
		...
		f.WriteLine "</table>"
			...
			'	end if
		f.WriteLine "<!--test25-->"
			' 15 OCT 24 LJL END check if any information in this section at all
			' 15 END OCT 24 OK
...
'		    end if
'			
'END no more law section for UN ------->			
...
'Atul make  colspan=7 from 6
			f.WriteLine "<td valign=""bottom"" class=""textitalic"" colspan=""7""><em>" & gITEXTT("i_4")& "</em></td>"
			f.WriteLine "</tr>"
...
' 08 MAR 11 LJL added UN Typing test section for UN agencies
	' if pv_new_sessioncode <> 3000 AND pv_new_sessioncode <> 7000 then
			f.WriteLine "<tr>"
			f.WriteLine "<td valign=""top"" colspan=""3"">" & gITEXTT("i_5")  & " " 

			' 30 MAY 11 LJL changed candlng_un_typ_i to a string instead of int
		 if JAPINFO1("candlng_typ_un_i") = "1" then
			   f.WriteLine "<strong>" & gITEXTT("i_6") & "</strong></td>"
			' 17 OCT 24 LJL only show UN typing results if yes to answer
			f.WriteLine "<td valign=""top"" colspan=""3"">" & gITEXTT("i_8") & " <strong>" & JAPINFO1("candlng_typ_un_y") & "</strong></td>"
			f.WriteLine "<td valign=""top"" colspan=""2"">" & gITEXTT("i_9") & " <strong>" & JAPINFO1("candlng_typ_un_t") & "</strong></td>"
		 else
			   f.WriteLine "<strong>" & gITEXTT("i_7")  & "</strong></td>"
			f.WriteLine "<td valign=""top"" colspan=""4""> </td>"  	
		end if
			f.WriteLine "</tr>"
	' end if
...
' 17 OCT 24 LJL marking where typing results are
	...
	f.WriteLine "<tr>"
	f.WriteLine "<td valign=""top"" colspan=""3"">" & gITEXTT("i_10") & "</td>"
	f.WriteLine "<td valign=""top"" colspan=""3"">" & gITEXTT("i_11") & "</td>"

	if pv_new_sessioncode = 2400 then
		f.WriteLine "<td valign=""top"" colspan=""1"">" & gITEXTT("i_21") & "</td>"
	else
		 f.WriteLine "<td valign=""top"" colspan=""1""> </td>"
	end if
...
' 17 OCT 24 LJL not show language typing lines if no entries
if JAPINFO1("candlng_en_typ_i") <> "" then
			f.WriteLine "<tr>"
...
' 17 OCT 24 LJL not show language typing lines if no entries
if JAPINFO1("candlng_fr_typ_i") <> "" then
			f.WriteLine "<tr>"
...
'Atul-  Add one column
	...
	' 17 OCT 24 LJL not show language typing lines if no entries
	if JAPINFO1("candlng_es_typ_i") <> "" then
			f.WriteLine "<tr>"
...
' Atul Add one new column
	...
	' 17 OCT 24 LJL not show language typing lines if no entries
	if JAPINFO1("candlng_ol_typ_t") <> "" then

			f.WriteLine "<tr>"
		'Atul make colspan=7 from from 4
			f.WriteLine "<td valign=""top"" colspan=""7"">" & gITEXTT("i_16") & "</td>"
			f.WriteLine "</tr>"
...
'set getRefList = rsys_db_select.execute(getRefListSql)
	'Added by atul for table of content TOC
		f.WriteLine "<p><br></p>"
...
' ******************************************************************************
   ...
   ' Response.Write "<br>T Diversity Section: " & pv_parts
'	Response.End
...
if instr(pv_parts,"DV,") then
	...
	Dim getDEIsql, getDEI
	Dim regex
		f.WriteLine "<!--Test93-->"
' 	Check if "Div," is present in pv_parts
	' 	Update the SQL query and variable names
	getDEIsql = "SELECT cand_div_keyPopulationsExplanation, cand_div_disability_accom, cand_div_other_raceethnicity, cand_div_other_genderidentity, cand_div_genderidentity, cand_other_gender, cand_other_pronoun, canddiv_pronoun_id, cand_div_race_ethnicity, cand_div_disability, cand_div_disability_accommodation, cand_div_key_population FROM tx_rsys_candmisc WHERE cand_id_c = " & applicant_id
...
'		    getRefList.Open getRefListSql, rsys_db_select, 1, 1
	...
	Set getDEI = Server.CreateObject("ADODB.RecordSet")
	rsys_db_select.CommandTimeout = 320
	' 	Open the recordset with the query
	getDEI.Open getDEIsql, rsys_db_select, 1, 1	   

		f.WriteLine "<!--Test94-->"

	' 	Check if there are any records returned

	If Not getDEI.EOF Then
		f.WriteLine "<!--Test94a-->"
...
' Single row for all questions and answers
f.WriteLine "<TR><td valign=""top"">"
'09 OCT 24 LJL use to test the section without query
'f.WriteLine "BUBBA</d></tr></table>"
...
'		Do While Not gITEXTDei.EOF
			' 	Write canddiv_pronoun_id with numbers and hyphens removed

		f.WriteLine "<!--Test94d-->"
			
			f.WriteLine "" & gITEXTDei("i_91") & ": "

' TESTED OUT START 1			   
' ---- OK 09 OCT 24		
			f.WriteLine "<!--Test95-->"
			
	If getDEI("canddiv_pronoun_id") = "0" Then
		f.WriteLine  "<b>" & getDEI("cand_other_pronoun") & " </b></td></tr>"
	End If
	   
	If Trim(regex.Replace(getDEI("canddiv_pronoun_id"), "")) <> "" Then
		f.WriteLine "<b>" & regex.Replace(getDEI("canddiv_pronoun_id"), "") & "</b></td></tr>"
	End If

  ' 		Write cand_gnd_i with numbers and hyphens removed
  ' 		f.WriteLine "<tr><td>" & gITEXTDei("i_87") & ": "      
...
'09 OCT 24 LJL added missing <tr>
		f.WriteLine  "<tr>"			


' 	03 OCT 24 LJL revised gender output for Diversity
			If JAPINFO1("cand_gnd_i") = 1  then
				f.WriteLine "<td valign=""top"">" & gITEXTA("i_5") & " <b>" & gITEXTA("i_29") & ""
			elseif JAPINFO1("cand_gnd_i") = 0  then
				f.WriteLine  "<td valign=""top"">" & gITEXTA("i_5") & " <b>" & gITEXTA("i_30") & ""
			elseif JAPINFO1("cand_gnd_i") = 3  then
				f.WriteLine  "<td valign=""top"">" & gITEXTA("i_5") & " <b>" & gITEXTA("i_97") & ""
			end if
			
		If JAPINFO1("cand_gnd_i") = 3 and len(getDEI("cand_other_gender")) Then
			 f.WriteLine  " - " & getDEI("cand_other_gender") & " </b></td></tr>"
		else
			 f.WriteLine  "</b></td></tr>"			
		End If
...
' ---- OK 09 OCT 24			
		...
		f.WriteLine "<tr><td>" & gITEXTDei("i_9") & ": "

		If getDEI("cand_div_genderidentity") = "-1" Then
			f.WriteLine  "<b>" & getDEI("cand_div_other_genderidentity") & " </b></td></tr>"
		End If
				 
		f.WriteLine "<!--Test97-->"

			If Trim(regex.Replace(getDEI("cand_div_genderidentity"), "")) <> "" Then
				f.WriteLine "<b>" & regex.Replace(getDEI("cand_div_genderidentity"), "") & "</b></td></tr>"
			End If

			' 	Write cand_div_race_ethnicity with numbers and hyphens removed
			f.WriteLine "<tr><td>" & gITEXTDei("i_87") & ": "   
			If Trim(regex.Replace(getDEI("cand_div_race_ethnicity"), "")) <> "" Then
					f.WriteLine "<b>" & regex.Replace(getDEI("cand_div_race_ethnicity"), "") & "</b>"
	   
					If InStr(getDEI("cand_div_race_ethnicity"), "-1") > 0 Then
						f.WriteLine "<b>" & regex.Replace(getDEI("cand_div_other_raceethnicity"), "") & "</b>"
					 End If
				 f.WriteLine " </td></tr>"
			End If
...
' ---- OK 09 OCT 24
		f.WriteLine "<!--Test98-->"
			...
			' 23 OCT 24 LJL disability 0 = no  1 = yes            
			f.WriteLine "<tr><td>" & gITEXTDei("i_text3") & " " 
		
		If getDEI("cand_div_disability") = "0" Then
				f.WriteLine "<b>" & gITEXTDei("i_32")	 & "</b></td></tr>"
		elseif getDEI("cand_div_disability") = "1" Then
				f.WriteLine "<b>" & gITEXTDei("i_31")	 & "</b></td></tr>"
		End If

' 23 OCT 24 LJL show disability need accommodations no matter what               
'        If getDEI("cand_div_disability") = "0" Then

			 f.WriteLine "<tr><td>" & gITEXTDei("i_83") & " "
			 If getDEI("cand_div_disability_accommodation") = "0" Then
					f.WriteLine "<b>" & gITEXTDei("i_32")	 & "</b></td></tr>"
			 elseif getDEI("cand_div_disability_accommodation") = "1" Then
				 f.WriteLine "<b>" & gITEXTDei("i_31")	 & "</b></td></tr>"
			  End If
...
' ---- OK 09 OCT 24
		...
		' 23 OCT 24 LJL disability 0 = no  1 = yes            
		' If getDEI("cand_div_disability") = "1" Then
		' 23 OCT 24 LJL show disability accommodations text if said yes
		'	If getDEI("cand_div_disability_accommodation") = "1" Then
				' 	Write cand_div_disability_accom with numbers and hyphens removed
				' 28 OCT 24 LJL if any accommodation text, print out
				If trim(regex.Replace(getDEI("cand_div_disability_accom"), "")) <> "" Then
					f.WriteLine "<tr><td>" & gITEXTDei("i_84") & ": <b>" & regex.Replace(getDEI("cand_div_disability_accom"), "") & "</b></td></tr>"
				End If
		'	End If
		' End If

'15 OCT 24 LJL don't show diversity populations if nothing in it.
if len(trim(getDEI("cand_div_keyPopulationsExplanation"))) then
	   f.WriteLine "<tr><td>" & gITEXTDei("i_97") & "<br><b>" & regex.Replace(getDEI("cand_div_keyPopulationsExplanation"), "") & " </b></td></tr>"
end if
' TESTED OUT END 3
...
JAPYsql = "SELECT upd_d, cand_fnam_t, cand_lnam_t, cand_fil_t, cand_fil_d, cand_verif_name_t, cand_law_i, cand_law_m," & _
		  ...
		  " cand_sexual_i, cand_sexual_m, cand_terminated_i, cand_terminated_m" & _
...
'response.write "<br>SQL: " & JAPYsql & "<br>"
...
f.WriteLine "<br>"
if trim(JAPY("cand_terminated_i")) = "1" then  
...
' End of single row for questions and answers
f.WriteLine "</td></TR>"
...
f.WriteLine "</TABLE>"
		...
		f.WriteLine "<!-- FOOTER CENTER """ & VACNUM & """ -->"
		f.WriteLine "<!-- NEW PAGE -->"
'		f.WriteLine "///NEW PAGE///"

		'06 OCT 24 LJL added page break css to force a break between applicant sections		
		'			f.WriteLine "<div style=""display: block; clear: both; page-break-after: always;""></div>"
		'			f.WriteLine "<div style=""height: 1px; overflow: hidden; page-break-after: always;""></div>"		
		'			f.WriteLine "<hr style='page-break-after: always; border: none; height: 0;'></hr>"
		
		'	f.WriteLine "<div class=""page-break""></div>"
		'	document.NewPage()
...
set obj_int_select_CmdDV = nothing
set obj_int_select_CmdII = nothing
