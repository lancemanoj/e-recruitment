...
'<<--Modified by Interface on 07/25/2007
	gITEXTVsql = "SELECT i_1, i_text1, i_3, i_4, i_10 FROM tr_rsys_itext WHERE itext_thisorg_c = ? AND itext_lng_c = ? AND itext_page_c = 'V' "
...
' INFO -- HAVE TO CHANGE THIS TO MAKE IT WORK WITH EACH SITE ADD ORG INFO
...
'response.write "LNGGG:" & new_lng_code
'response.end
...
'22 Nov 06 ac -  - debug flag - could be moved to page header for all files?
if pv_DEBUG then
	response.write "<br>acdoc-maker.asp"
...
if pv_multicode <> "" OR applicant_id <> "" then
	...
	response.write "<Br><font color='maroon'>IMPROPER ENTRY - NO CAND ID, please report how you entered this page to tech support <a href='mailto:erecruit@talentisoft.com'>Tech Support</a></font>"
	response.end
end if


'response.buffer = false
' response.write "BEGIN 1"
...
'response.write "<br> TESTOUT: EEE pv_usepix:=" &pv_usepix
...
'19 DEC 06 LJL added to move head and body here
...
' 17 APR 07 LJL modif to properly assess if truly internal, mainly WHO  - thisorg_stafftrue is the staff number from WHO. If somethng there, then they are in WHO internal dir
'09 MAY 07 LJL modified this section for WHO and others, for internal data
if false then '(pv_new_sessioncode = 1000)   OR pv_new_sessioncode = 1500 by Gevorg
' 17 APR 07 LJL modif to properly assess if truly internal, mainly WHO  - thisorg_stafftrue is the staff number from WHO. If somethng there, then they are in WHO internal dir

    if len(JAPINFO1("thisorg_stafftrue")) then
'f.WriteLine "GOOBER222"
'STAFFTRUE: " & JAPINFO1("thisorg_stafftrue") & ":: SESSION: " & pv_new_sessioncode
        
        
        f.WriteLine "<a  style=""abcpdf-tag-visible: true;"" id='" & trim(Ucase(JAPINFO1("lastname")))  & ", " & JAPINFO1("firstname") & "_|"  & intNumber  &  "_h1'></a>" 
		f.WriteLine trim(Ucase(JAPINFO1("lastname")))  & ", " & JAPINFO1("firstname")
	else
'f.WriteLine "GOOBER111"
        f.WriteLine "<a  style=""abcpdf-tag-visible: true;"" id='" & trim(Ucase(JAPINFO1("cand_lnam_t"))) & ", " & JAPINFO1("cand_fnam_t") & "_|"  & intNumber  & "_h1'></a>" 
...
'    .Charset = "ascii"
'    .Open
'    .WriteText JAPINFO1("cand_fnam_t")
'    .Position = 0
'    'response.write .ReadText
'    pv_newFname = .ReadText
'End With

'With Server.CreateObject("Adodb.Stream")
'    .Charset = "ascii"
'    .Open
'    .WriteText JAPINFO1("cand_lnam_t")
'    .Position = 0
'    'response.write .ReadText
'    pv_newLname = .ReadText
'End With

			ApplicantName = trim(Ucase(pv_newLname)) & "_" & pv_newFname
			'ApplicantName = trim(Ucase(JAPINFO1("cand_lnam_t"))) & ", " & JAPINFO1("cand_fnam_t")
		End If
end if
f.WriteLine "</h3>"

'response.write "<br><br>NAME1: " & .ReadText
'response.write "<br><br>NAME2: " & pv_newname
'response.write "<br><br>NAME3: " & JAPINFO1("cand_fnam_t")
'response.write "<br><br>ENDED"
'response.end


'f.WriteLine "<th colspan=""2""><font size=" & pv_THfontsize & ">BLAH</th>"

'f.WriteLine "<td><th colspan=""2""><font size=" & pv_THfontsize & ">" & gITEXTPH("i_2") & day(now()) & " " & monthname(month(now())) & " " & year(now()) & "</th></td></tr></table>"

'response.write "TESTOUT: FFF pv_pixok" & pv_pixok
...
'response.write "TESTTTT AAAA<Br>"
''-----------------------------------------NOTES ----------
...
'Modified from appA-view.asp
'Modified by LJL (SECANTSYS) lance.lundberg@dock.net
'18 JUN 04 LJL changed countries to be more in line
'MODS --
'23 FEB 05 LJL org check
'4 MAR 05 LJL staff number not removed for WTO, only hidden because none will be entered
'--------------------------------------------------------------->
'4 MAR 05 LJL staff number not removed for WTO, only hidden because none will be entered
'--------------------------------------------------------------->

'19 DEC 06 LJL added to show name and section better with <h header bits
	f.WriteLine "<br/>"
...
'Change by Atul
	...
	<!--f.WriteLine "<tr><th colspan=""2"">  <a style=""abcpdf-tag-visible: true;"" id='" & gITEXTA("i_1") & "_|" & intNumber & "' > " & gITEXTA("i_1") &  "123</a></th></tr>"-->
	
	
'f.WriteLine "<tr><td colspan=""2"" align=""left"" bgcolor=""" & gITEXTPH("i_60") & """><h2><font color=""" & g_headerColor & """>" & gITEXTA("i_1") & "</font></h2></td></tr>"
	
  
'if multi <> "" then
'	if JAPINFO("thisorg_short") > "0" AND (new_session_code = 1000 OR new_session_code = 1500) then
'		f.WriteLine Ucase(JAPINFO("lastname"))  & " " & JAPINFO("firstname")
'	else
'		f.WriteLine Ucase(JAPINFO("cand_lnam_t")) & " " & JAPINFO("cand_fnam_t")
'	end if
'	f.WriteLine " - " & gITEXTA("i_1")
'else
'	f.WriteLine " " & gITEXTA("i_1")
'end if
'f.WriteLine "</th></td></tr>"
			if pv_DEBUG then
...
'	src_pix_loc = "E:\stage\" & pv_new_sessioncode & "_docs\vac-cv\"
'else
'	' new folder location repalces docstore/vac-cv while in live
'		src_pix_loc = "E:\docs\" & pv_new_sessioncode & "_docs\vac-cv\"
'	end if
...
'30 JAN 15 LJL added UPU to image list
'				if pv_new_sessioncode = 1000 or pv_new_sessioncode = 1500 or pv_new_sessioncode = 2000 or pv_new_sessioncode = 2400 or pv_new_sessioncode = 2800 or pv_new_sessioncode = 2900 or pv_new_sessioncode = 5500 then
'					src_pix_loc = "E:\docs\stage\UNSHARE\vac-cv\"
'new folder location for IFRC
'				elseif pv_new_sessioncode = 7000 then
'					src_pix_loc = "E:\docs\stage\IFRC\vac-cv\"
'new folder location for WTO
'				elseif pv_new_sessioncode = 3000 then
'					src_pix_loc = "E:\docs\stage\WTO\vac-cv\"
'				end if
...
'14 SEP 10 LJL removed contract details other than contract type, for ITU
			if pv_new_sessioncode <> 7000 AND pv_new_sessioncode <> 3000 AND pv_new_sessioncode <> 2400 AND pv_new_sessioncode <> 2800 then
				if pv_new_sessioncode = 1000 then
					if JAPINFO1("thisorg_stafftrue") > 0 then
						f.WriteLine "<TR>"
						f.WriteLine "<td valign=""top"">" & gITEXTA("i_68") & " <strong>"
						...
						f.WriteLine "</strong></td>"
						f.WriteLine "<td valign=""top"">" & gITEXTA("i_69") & " <strong>"
...
'Add by Atul
			...
			f.WriteLine "<TABLE border=""0"" bordercolor=""black"" cellpadding=""2"" width=100% align=""center"">"
...
'f.WriteLine "<TR><td valign=""top"" colspan=""3"" bgcolor=""" & gITEXTPH("i_60") & """><h2><font color=""" & g_headerColor & """>" & gITEXTS("i_1") & "</font></h2></TD></tr>"
			...
			if not gAOE.eof then
				...
				Do while not gAOE.eof
					...
					f.WriteLine "<td valign=""top"" align=""left""> <strong>" & gAOE("ccogdsc") & "</strong>"
...
'f.WriteLine "<h2><font color=""" & g_headerColor & """>" & gITEXTE("i_1") & "</font></h2></TD></tr>"
		...
		f.WriteLine "<TR><td valign=""top"" class=""teal10"" colspan=""3""><font color=""Teal""><b><i>" & gITEXTE("i_37")& "</i></b></font></TD>"
		f.WriteLine "<td valign=""top"" colspan=""1""><b>" & JAPEDU("edddsc") & "</b></TD></tr>"
...
'10 JUN 15 GG Changed i_39 to i_14 and moved here	
		f.WriteLine "<tr><td valign=""top"" colspan=""4""><TABLE>"
...
''------ insertion ------->			
				f.WriteLine "<TR><td valign=""top"" colspan=""4""><table  cellpadding=""1""><tr>"
				f.WriteLine "<td width=""15%""  valign=""top"" ><u><i>" & gITEXTF("i_27") & "</I></u></td>"
				f.WriteLine "<td  width=""83%"" valign=""top""><b>" & JAPEMP("candemploy_title_t") & "</b></td>"
		...
		'11 JUN 15 GG Added EMPLOYMENT HISTORY Areas of Expertise for WMO	
			...
			if pv_new_sessioncode <> 7000 then ' NOT FOR IFRC SALARY
				f.WriteLine "<TR><td valign=""top"" colspan=""4""><table  cellpadding=""1""><tr>"
				f.WriteLine "<td width=""15%"" valign=""top"" ><i>" & gITEXTF("i_60") & " " & gITEXTF("i_45") & " " & gITEXTF("i_56") & " " & gITEXTF("i_63") & "</I></td>"
				f.WriteLine "<td align=""left"" width=""40%"" valign=""top"" ><b>" & JAPEMP("candemploy_sal1_c") & "</b></td>"
				f.WriteLine "<td width=""15%"" valign=""top"" ><i>" & gITEXTF("i_62") & " " & gITEXTF("i_45") & " " & gITEXTF("i_56") & " " & gITEXTF("i_63") & "</I></td>"
...
'f.WriteLine "</TR>"
			...
			f.WriteLine "<TR><td valign=""top"" colspan=""4""><table cellpadding=""0""><tr>"
...
Dim pv_rower_comp
	...
	Do while JAPCOMPS.eof = false
		...
		f.WriteLine "<td><table><tr><td>" & JAPCOMPS("q_elemrem") & "<br><font color='blue'>&nbsp; "
...
'02 JUN 15 GG changed References section look like other sections				
		   ...
		   ' else
		    ...
		    loop
'01 JUN 15 LJL made table 100% to fill page across for In city since, in country since
			f.WriteLine "</table><table width=100% border=0 bordercolor=green>"
...
' END CHECK IF PERSON SELECTED SECTION H
		...
		f.WriteLine "<!--Test38-->"
' ******************************************************************************
' SECTION COVERLETTER type 13 POST SPECIFIC
' ******************************************************************************
...
'ac - removedf.WriteLine "<TABLE border=""0"" bordercolor=""black"" cellpadding=""2"" width=""100%"" align=""center"" border=""0"">"
		...
		f.WriteLine "<TABLE border=""0"" bordercolor=""black"" cellpadding=""2"" width=""" & widther & """ align=""center"" border=""0"">"
...
' BEGIN CHECK IF PERSON SELECTED SECTION OI
		f.WriteLine "<!--Test43-->"
		// Added by DD, 04/02/2009 for WTO
       //if instr(pv_parts,"OI,") then
